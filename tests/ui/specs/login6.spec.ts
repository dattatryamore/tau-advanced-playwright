...
test.describe('Book Store - Login 6', () => {
  test(`successfull login 6`, async () => {
    await loginPage.doLogin(userName, password);
    await loginPage.checkLoggedIn();
    
  });

  test(`failing login 6- invalid username`, async () => {
    let invalidUsername = userData.invalidUsername;
    await loginPage.doLogin(invalidUsername, password);
    await loginPage.checkInvalidCredentials();
  });

  test(`failing login 6- invalid password`, async () => {
